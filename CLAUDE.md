...
- **Fixed jq parse errors from bashio library**: Implemented global stderr filter and replaced bashio::cache with simple file-based caching. Base image 19.0.0 uses jq 1.8.1 which has breaking changes (tonumber/0 rejects whitespace, stricter JSON parsing). Bashio cache operations trigger "Invalid numeric literal" errors which are now completely suppressed via stderr filtering. Added retry logic for addon status checks.
